...
parser.add_argument('--model_dir', default=r"C:\Work\2022_workstuff\Research\anomalyDetectors\MNAD\log_dir\avenue_deep2_pred\model_59.pth", type=str, help='directory of model')
parser.add_argument('--m_items_dir', default=r"C:\Work\2022_workstuff\Research\anomalyDetectors\MNAD\log_dir\avenue_deep2_pred\keys_59.pt", type=str, help='directory of model')
...
if __name__ == "__main__":
    ...
    for k,(imgs) in enumerate(test_batch):
        ...
        main_save_dir = r"D:\2022_workstuff\Research\anomalyDetectors\MNAD\result_images"
            
        parts_type = videos_list[video_num].split('/')[-3]
        parts_num_dir = videos_list[video_num].split('/')[-1]

        save_dir = os.path.join(main_save_dir, f"{args.dataset_type}_{args.method}",f"{parts_type}","frames", f"{parts_num_dir}")
        ...
        if args.method == 'pred':
            ...
            # full_img = loss_func_mse((outputs[0]+1)/2, (imgs[0,3*4:]+1)/2)
            inp = (imgs[0,3*4:]+1)/2
            rec = (outputs[0] + 1) / 2
            ...
            # full_img = loss_func_mse((outputs[0]+1)/2, (imgs[0]+1)/2)
            mse_feas = compactness_loss.item()
